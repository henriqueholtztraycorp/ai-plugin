...
This repo is a **Claude/Cursor plugin** — declarative, markdown-only content in `agents/`, `commands/`, `skills/`, `rules/`, `.claude-plugin/`, `.cursor-plugin/`, `assets/`. No build step, no compiled artifacts.
...
- **Forbidden in plugin content:** any reference to `api.fbits.net` (or `*.fbits.net`). Markdown under `skills/`, `agents/`, `commands/`, `rules/` must point users at the canonical docs site `https://wakecommerce.readme.io/docs/schema` instead.
